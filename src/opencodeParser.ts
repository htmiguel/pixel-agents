...
import { PERMISSION_EXEMPT_TOOLS,processTranscriptLine } from './transcriptParser.js';
    ...
    if (record.role === 'user') {
      ...
      if (Array.isArray(content)) {
        const hasToolResult = content.some(
          (b: Record<string, unknown>) => b.type === 'tool_result' || b.type === 'function_result',
        );
        if (hasToolResult) {
          for (const block of content) {
